...
> Note: Clipy itself was revived in **v1.3.0 (2026-06-20)** with native Apple Silicon support. Both are good, free, open-source tools — Tameo is for people who specifically want history search, image OCR, and a fully local app that phones nowhere.
...
- **Nothing phones home.** No servers, accounts, analytics, telemetry, or Firebase — nothing leaves your Mac. The only network call Tameo ever makes is its Sparkle update check. ([Privacy Policy](PRIVACY.md))
- **History is encrypted at rest.** Everything content-bearing in the history database is AES-GCM-encrypted with a key kept in your Keychain. Always on — there is no off switch.
- **Passwords stay out.** Concealed items (`org.nspasteboard.ConcealedType`) from password managers are never saved, and that can't be switched off.
...
- Privacy-aware monitoring: change detection never reads contents, and type/app/marker filtering happens before any content is read
...
- **Pin** favorites to the top (⌘P), **delete** a single item (⌘⌫), and **filter by type** (text/rich/image/file/URL/color)
...
- **⌃ transform paste** — paste with fixes applied: full-width → half-width, URL tracking-parameter removal (utm_*, fbclid…), and whitespace tidy. Choose which in Settings ▸ General. Your history is never modified
- **Encrypted history** — the history database is always encrypted at rest with AES-GCM (key in your Keychain), no off switch
- Full settings: history size, data-type toggles, hotkeys, login-at-startup, app exclusions

> **New to Tameo? See the [Usage Guide](https://tameo.ati-mirai.co.jp/guide/)** — palette, the three paste modes (number / ⌥ plain / ⌃ transform), search, OCR, snippets, and privacy.
...
Clipy は優れたツールです。Tameo はその挙動・機能だけを参考に、コードは流用せずモダンな構成（SwiftUI／SwiftData）でクリーンルーム実装したもので、自分が欲しかった **履歴の全文検索・画像内テキストの OCR 検索** を足し、**解析・テレメトリSDK（Firebase等）を一切含まない完全ローカル**にしています。

> 補足：Clipy 自身も **v1.3.0（2026-06-20）** で開発が再開し、Apple Silicon ネイティブ対応が入りました。どちらも無償・OSS の良いツールで、Tameo は「履歴検索・画像OCR・どこへも通信しないローカル」が欲しい人向けの選択肢です。

- **画像の中の文字まで検索**：入力するだけで履歴を絞り込み、オンデバイスOCRでコピー画像・スクショ内の文字も検索・貼付（完全ローカル）。Clipy にはどちらもまだありません
- **どこへも通信しない**：サーバー・アカウント・解析・テレメトリ・Firebase なし。端末から何も出ません。唯一の通信は Sparkle の更新確認だけです（[プライバシーポリシー](PRIVACY.md)）
- **履歴は暗号化して保存**：履歴データベースの内容はすべて AES-GCM で暗号化され、鍵はあなたのキーチェーンに保管されます。常時有効でオフにはできません
- **パスワードは溜めない**：パスワードマネージャの機密項目（`org.nspasteboard.ConcealedType`）は保存しません。これは設定で無効化できません
- **読む前に弾く**：まずペーストボードの「種別」だけを確認し、機密項目・除外アプリ・オフにした種別は**中身を読む前に**除外します。内容の読み取りは[検証可能な1箇所](Tameo/Services/ClipboardMonitor.swift)に集約しています
...
- ⌥ でプレーンテキスト貼付（装飾を捨てる）
- **⌃ で変換貼付** — 全角→半角・URLのトラッキングパラメータ除去（utm_* など）・空白整理をかけて貼り付け。使う変換は 設定 ▸ General で選択。履歴そのものは変更しない
- **履歴の暗号化** — 履歴データベースは AES-GCM で常時暗号化して保存（鍵はキーチェーン）。オフ設定なし
- 設定：履歴件数／種別ごと保存／ホットキー／ログイン時起動／除外アプリ

> **はじめての方は [使い方ガイド](https://tameo.ati-mirai.co.jp/guide/) をご覧ください** — パレット、3つの貼り付け方（番号／⌥平文／⌃変換）、検索・OCR・スニペット・プライバシーまで。
