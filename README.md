...
> Status: **Active development (v0.1.0).** All clipboard types, paste, snippets, and settings work today. A signed & notarized release is in preparation.
> 状態: **開発中（v0.1.0）。** 履歴・ペースト・スニペット・設定は動作します。署名・公証済みの配布版を準備中です。

🌐 Website: https://tameo.ati-mirai.co.jp  ·  📜 [Privacy Policy](PRIVACY.md)  ·  ⚖️ [MIT License](LICENSE)
...
- **No nagging.** Designed around the newer macOS pasteboard-privacy APIs, so it doesn't trigger "an app read the clipboard" warnings.
...
> A signed, notarized build is being prepared. Until it ships, build from source (below).

- **Direct download (notarized DMG):** https://github.com/supergodak/tameo/releases
- **Homebrew:** `brew install --cask tameo` _(coming soon)_
...
- Auto-update: [Sparkle](https://github.com/sparkle-project/Sparkle) (planned)
...
- **警告を出さない**：新しい macOS のペーストボード・プライバシーAPIに対応
...
> 署名・公証済みビルドを準備中です。それまではソースからビルドしてください（下記）。

- **直接ダウンロード（公証済みDMG）:** https://github.com/supergodak/tameo/releases
- **Homebrew:** `brew install --cask tameo` _(準備中)_
