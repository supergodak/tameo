...
**The history database is encrypted at rest.** Every content-bearing field (text, search index, OCR text, image originals and thumbnails, file paths, color codes) is encrypted with AES-GCM before it touches disk. The encryption key is generated on your Mac and kept in your login Keychain — it never leaves the device. This is always on and cannot be switched off. When you update from an older version, existing plaintext history is encrypted once at first launch and the database file is rebuilt so no plaintext remnants remain.
...
**履歴データベースは保存時に暗号化されます。** 内容を持つすべてのフィールド（本文・検索索引・OCRテキスト・画像の原本とサムネイル・ファイルパス・カラーコード）は、ディスクに書かれる前に AES-GCM で暗号化されます。暗号鍵はあなたの Mac 上で生成され、ログインキーチェーンに保管されます（端末の外へは出ません）。この暗号化は常時有効で、オフにはできません。旧バージョンからの更新時は、初回起動時に既存の平文履歴を一度だけ暗号化し、データベースファイルを再構築して平文の残骸も残しません。
