...
final class SettingsStoreTests: XCTestCase {
    ...
    /// 機密マーカーは設定で無効化できない＝常に除外マーカー集合へ入っていること。
    /// （README / PRIVACY.md の「パスワードは never saved」を満たす回帰ガード）
    func test_concealedMarker_isAlwaysIgnored_andNotConfigurable() {
        XCTAssertTrue(alwaysIgnoredMarkerTypes.contains(.concealed))
        XCTAssertTrue(alwaysIgnoredMarkerTypes.contains(.transient))
        XCTAssertTrue(alwaysIgnoredMarkerTypes.contains(.autoGenerated))
    }
